...
class PPOAgent(Common_Methods):
    def __init__(self, buffer_size=512, input_dim=4, hidden_dim=128, actor_lr=1e-3, critic_lr=1e-3, gamma=0.99, clip_value=0.2, lambda_gae=0.95, entropy_bonus=False):
    ...
    def learn_ppo(self, last_state):        
        ...
        if self.ent_bonus :
            ...
            actor_loss = -torch.min(surr1, surr2).mean() - self.c1 * entropy # PPO objective (to maximize) with entropy bonus
            critic_loss = self.c2 * self.loss_fct(new_values, returns)
        else :
            actor_loss = -torch.min(surr1, surr2).mean() # PPO objective (to maximize) without entropy bonus
            critic_loss = self.c1 * self.loss_fct(new_values, returns)
